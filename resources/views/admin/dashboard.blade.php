...
@section('content')
        ...
        <div class="mb-6 space-y-2">
            @foreach($alerts as $alert)
                <div class="bg-red-50 border-l-4 border-red-600 p-4 rounded shadow-sm flex items-start gap-3">
                    ...
                    <div>
                        <h3 class="text-red-800 font-bold">Bezpečnostné varovanie</h3>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            <p class="text-gray-500 text-sm uppercase font-semibold tracking-wider">Aktívnych Riadkov</p>
            <p class="text-3xl font-bold text-[#722F37] mt-1">{{ count($rows) }}</p>
        </div>
        <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            <p class="text-gray-500 text-sm uppercase font-semibold tracking-wider">Najbližší zber</p>
        ...
        <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 bg-gradient-to-br from-[#4A5D23] to-[#3a4a1b] text-white">
            <p class="opacity-80 text-sm uppercase font-semibold tracking-wider">Počasie (Vinica)</p>
            <div class="flex items-center gap-3 mt-2">
                <span class="text-3xl font-bold">24°C</span>
                <span class="text-sm opacity-90">Slnečno</span>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="px-6 py-5 border-b border-gray-100 flex justify-between items-center">
            <h3 class="font-bold text-lg text-gray-800">Stav Vinohradníckych Riadkov</h3>
            <span class="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded">Sezóna 2025</span>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                <thead class="bg-gray-50 text-gray-500 uppercase text-xs font-semibold">
                    <tr>
                        <th class="px-6 py-4">Označenie</th>
                        <th class="px-6 py-4">Odroda</th>
                        <th class="px-6 py-4">Posledné ošetrenie</th>
                        <th class="px-6 py-4">Plánovaný zber</th>
                        <th class="px-6 py-4">Stav</th>
                        <th class="px-6 py-4 text-right">Akcia</th>
                    </tr>
                </thead>
                        ...
                        <td class="px-6 py-4">
                            @if($row['status'] == 'warning')
                                <span class="inline-flex items-center gap-1 text-red-600 bg-red-50 px-2 py-1 rounded text-xs font-bold">
                                    <span class="w-2 h-2 bg-red-600 rounded-full animate-pulse"></span> Riziko
                                </span>
                            @else
